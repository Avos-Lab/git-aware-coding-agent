"""Session-ask command orchestrator for AVOS CLI.

Implements the `avos session-ask "question"` flow: hybrid search over
Memory B (session/live), sanitizes, packs within budget, synthesizes via
LLM, validates citation grounding, and renders answer or deterministic fallback.
"""
...
_log = get_logger("commands.session_ask")

_SESSION_ASK_K = 10
_SESSION_ASK_SEARCH_MODE = "hybrid"
    ...
    if reply_service:
            ...
            if json_str:
                ...
                print_json(
                    ...
                    error={
                        ...
                        "message": "Could not format JSON output.",
                        "hint": None,
                        "retryable": True,
                    },
                )
        ...
        if json_output:
            print_json(
                ...
                error={
                    "code": "REPLY_SERVICE_UNAVAILABLE",
                    "message": "JSON output requires REPLY_MODEL environment variables.",
                    "hint": "Set REPLY_MODEL, REPLY_MODEL_URL, and REPLY_MODEL_API_KEY.",
                    "retryable": False,
                },
            )
        else:
...
class SessionAskOrchestrator:
    """Orchestrates the `avos session-ask` command.

    Pipeline: hybrid search over Memory B -> sanitize -> budget ->
    synthesize -> ground -> render/fallback.
    ...
    def run(self, repo_slug: str, question: str, json_output: bool = False) -> int:
        """Execute the session-ask flow.

        Args:
            repo_slug: Repository identifier in 'org/repo' format.
            question: Natural language question about session/live context.
        ...
        # Stage 1: Retrieve (hybrid search over Memory B)
        try:
            search_result = self._memory.search(
                memory_id=memory_id,
                query=question,
                k=_SESSION_ASK_K,
                mode=_SESSION_ASK_SEARCH_MODE,
            )
        except AvosError as e:
            self._emit_error(e.code, f"Memory search failed: {e}", retryable=True)
            return 2
        ...
        if not search_result.results:
            if json_output:
                print_json(
                    success=True,
                    data={"answer": None, "evidence": [], "message": "No matching evidence found."},
                    error=None,
                )
            else:
                print_info(
                    "No matching evidence found in session memory. "
                    "Try 'avos session-start' or run 'avos session-end' first."
                )
            return 0
